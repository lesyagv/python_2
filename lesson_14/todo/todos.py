...
class TodoList:
    ...
    def delete(self, position: int) -> DBResponse:
        """Delete todo"""

        reader = self._db_handler.read_todos()

        if position < 1 or position > len(reader.todo_list):
            return DBResponse(reader.todo_list, f"Invalid position {position} for deletion")

        del reader.todo_list[position - 1]
