...
def index():
    ...
    "SELECT emp.id emp_id, employee_name, department_id, department_name, join_date, role_id, "
    "role_name FROM employee emp JOIN roles r ON emp.role_id = r.id JOIN department d ON emp.department_id = d.id "
...
def create():
    ...
    if request.method == "POST":
        ...
        if role_id in (1, 3):
            db.execute(
                "INSERT INTO employee (employee_name, weekly_salary, role_id, department_id) VALUES (?, ?, ?, ?)",
                (employee_name, weekly_salary, role_id, department_id)
            )
            db.commit()
        
        elif role_id == 2:
            db.execute(
                "INSERT INTO employee (employee_name, weekly_salary,commission_per_sale, role_id, department_id) VALUES (?, ?, ?, ?, ?)",
                (employee_name, weekly_salary, commission_per_sale, role_id, department_id)
            )
            db.commit()

        elif role_id == 4:
            db.execute(
                "INSERT INTO employee (employee_name, hour_rate, role_id, department_id) VALUES (?, ?, ?, ?)",
                (employee_name, hour_rate, role_id, department_id)
            )
            db.commit()
